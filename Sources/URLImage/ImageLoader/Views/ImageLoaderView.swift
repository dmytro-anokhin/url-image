...
struct ImageLoaderView<Content, Placeholder> : View where Content : View, Placeholder : View {
    ...
    init(_ urlRequest: URLRequest, delay: TimeInterval, incremental: Bool, animated: Bool, expireAfter expiryDate: Date? = nil, processors: [ImageProcessing]?, services: Services, placeholder: @escaping (_ downloadProgressWrapper: DownloadProgressWrapper) -> Placeholder, content: @escaping (_ imageProxy: ImageProxy) -> Content) {
        ...
        // model = Model(urlRequest, delay: delay, incremental: incremental, expireAfter: expiryDate, processors: processors, services: services)
    }

    // @ObservedObject private var model: Model

    var body: some View {
//        let model = ImageLoaderContentView<Content, Placeholder>.Model(urlRequest, delay: delay, incremental: incremental, expireAfter: expiryDate, processors: processors, services: services)
////
//        return ImageLoaderContentView(model: model, placeholder: placeholder, content: content)
////
        let model = Model(urlRequest, delay: delay, incremental: incremental, expireAfter: expiryDate, processors: processors, services: services)
...
//    var body: some View {
//        ImageLoaderContentView(model: contentViewModel, placeholder: placeholder, content: content)
//            .onAppear {
//                self.services.fileDownloadService.downloadFile(with: self.urlRequest.url!) { _ in
//                }
//            }
//            .onDisappear {
//            }
